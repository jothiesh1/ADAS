...
class DrowsinessDetector:
    ...
    def process_frame(self, frame):
        ...
        if ear < EYE_AR_THRESH:
            ...
            if self.eye_closed_start and (current_time - self.eye_closed_start >= EYE_CLOSED_DURATION):
                if current_time - self.last_eye_closure_time >= EYE_CLOSED_DURATION:
        ...
        if mar > MOUTH_AR_THRESH:
            self.yawn_counter += 1
            #print(f"MAR: {mar}")  # Debugging: print the MAR value
            if self.yawn_counter >= MOUTH_AR_CONSEC_FRAMES:
                if current_time - self.last_yawn_time >= 3.0:  # Minimum 3 seconds between yawn signals
                    print("[INFO] Yawn detected!")  # Debugging: print when a yawn is detected
