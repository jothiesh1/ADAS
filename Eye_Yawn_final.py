    ...
    while True:
        ...
        if mar > MOUTH_AR_THRESH:
            ...
            if mouth_counter >= MOUTH_AR_CONSEC_FRAMES:
                total_yawns += 1
                if time.time() - last_yawn_time > 3:  # Send data only once every 3 seconds
        ...
        if total_yawns >= WARNING_YAWN_COUNT:
            if time.time() - last_yawn_time > 3:  # Send data every 3 seconds after yawning detection
                threading.Thread(target=send_serial_data, args=(25,)).start()
                last_yawn_time = time.time()
        elif eye_closed_start_time and (current_time - eye_closed_start_time >= EYE_CLOSED_DURATION):
            if time.time() - last_eye_close_time > 3:  # Prevent repeat sending for prolonged eye closure
                threading.Thread(target=send_serial_data, args=(25,)).start()
