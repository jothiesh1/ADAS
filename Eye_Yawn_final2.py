...
def send_signal_limited(value):
    ...
    if len(last_signal_times) < 1:
        threading.Thread(target=send_serial_data, args=(value,)).start()
...
def main():
    ...
    while True:
        ...
        if mar > MOUTH_AR_THRESH:
            ...
            if mouth_counter >= MOUTH_AR_CONSEC_FRAMES:
                ...
                if time.time() - last_yawn_time > 3:  # Only check yawning every 3 seconds
                    send_signal_limited(25)  # Use the new limited function
        ...
        if total_yawns >= WARNING_YAWN_COUNT:
            ...
            send_signal_limited(20)  # Use the new limited function
